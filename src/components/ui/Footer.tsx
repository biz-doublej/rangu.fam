...
const Footer: React.FC = () => {
  // DoubleJ 공식 사이트(doublej.app) 및 계정센터 링크
  const doublejLinks = [
    { label: 'DoubleJ 홈', href: 'https://doublej.app/' },
    { label: '서비스', href: 'https://doublej.app/#services' },
    { label: '역량', href: 'https://doublej.app/#capabilities' },
    { label: '팀', href: 'https://doublej.app/team' },
    { label: '문의', href: 'https://doublej.app/#contact' },
    { label: '회사소개', href: 'https://accounts.doublej.app/company' },
  ]
  ...
  const services = [
    ...
    { name: '이랑위키', description: '우리만의 기록을 모으는 위키', href: BRANDING.wikiPublicUrl },
        ...
        <div className="grid grid-cols-1 gap-10 md:grid-cols-2 lg:grid-cols-4">
          ...
          >
            ...
            <ul className="mt-4 flex flex-wrap gap-x-4 gap-y-1.5">
              {doublejLinks.map((item) => (
                <li key={item.label}>
                  <a
                    href={item.href}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="group inline-flex items-center gap-1 text-sm text-ink-300 transition-colors hover:text-coral-500"
                  >
                    {item.label}
            ...
            <a
              href="https://doublej.app/"
              target="_blank"
              rel="noopener noreferrer"
              className="mt-5 inline-flex items-center gap-2 rounded-full border border-ink-500/15 bg-paper-50 px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-ink-500 transition-colors hover:border-coral-500/40 hover:text-coral-500"
            >
