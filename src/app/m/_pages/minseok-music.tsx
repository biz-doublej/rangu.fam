...
export default function MinseokMusicPage({
  onSwitchToHospitality,
}: {
  onSwitchToHospitality?: () => void
}) {
  ...
  return (
    <div className="minseok min-h-screen bg-black text-white antialiased">
      ...
      <header className="mx-auto flex max-w-6xl items-center justify-between px-8 pt-8 text-xs uppercase tracking-[0.3em]">
        ...
        <button
          type="button"
          onClick={onSwitchToHospitality}
          className="group flex items-center gap-2 opacity-60 hover:opacity-100 transition"
        >
      ...
      <section className="border-t border-zinc-900">
        <div className="mx-auto max-w-6xl px-8 py-24">
          ...
          >
            <p className="font-mono text-xs uppercase tracking-[0.3em] text-zinc-500">— flip the record</p>
            <h2 className="mt-4 font-serif text-6xl transition group-hover:translate-x-2">
              side b — hospitality.
            </h2>
            <p className="mt-6 max-w-xl text-zinc-500">
              IMI Switzerland 재학 중인 호스피탈리티 학생으로서의 또 다른 포트폴리오.
              경력 · 자격 · 인턴십 지원 정보를 영문으로 정리해두었어요.
            </p>
            <span className="mt-8 inline-flex items-center gap-3 border-b border-white pb-1 font-mono text-xs uppercase tracking-[0.3em] transition group-hover:gap-5">
