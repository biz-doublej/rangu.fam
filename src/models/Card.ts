...
export interface ICard {
  ...
  imageUrl: string        // 카드 이미지 URL (또는 배경 이미지 URL)
  cardImageUrl?: string   // 실제 카드 이미지 URL (시그니처 카드 등에서 사용)
  
  // 카드별 특정 정보
  member?: string         // 멤버 이름 (개인 카드의 경우)
  year?: number          // 연도 (년도 카드의 경우)
  period?: 'h1' | 'h2'   // 상반기/하반기 (구 년도 카드의 경우)
  version?: number       // 버전 (새 년도 카드의 경우)
...
interface ICardDocument extends ICard, ICardMethods, mongoose.Document {}

// Card model interface
type CardModel = mongoose.Model<ICardDocument>
...
// Instance method to generate image URL
CardSchema.methods.generateImageUrl = function(): string {
  if (this.member) {
    return generateCardImageUrl(this.member, this.type, this.year, this.period)
  }
  // For group cards or cards without members
  return `/images/cards/${this.type}/${this.cardId}.jpg`
}

// Pre-save hook to auto-generate imageUrl if not provided
...
export const Card = mongoose.models.Card || mongoose.model<ICardDocument>('Card', CardSchema)
