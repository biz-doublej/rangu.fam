...
export class CardService {
  ...
  static async createDefaultCard(): Promise<any> {
    ...
    try {
      // 랜덤으로 멤버와 연도 선택
      const members = ['재원', '강한울', '이승찬', '정민석', '정진규']
      ...
      // 멤버 약어 매핑
      const memberAbbrevMap: Record<string, string> = {
        '재원': 'JAE',
        '강한울': 'HAN',
        '이승찬': 'LEE',
        '정민석': 'MIN',
      ...
      const defaultCard = new Card({
        ...
        description: `${randomYear}년 ${randomPeriod === 'h1' ? '상반기' : '하반기'} ${randomMember}의 모습`,
        imageUrl: `/images/cards/year/${memberAbbrev}_${randomYear}_${randomPeriod}.jpg`,
        member: randomMember,
        year: randomYear,
        period: randomPeriod,
        isGroupCard: false,
      ...
      // 메모리상 임시 카드도 랜덤으로 생성
      const randomMember = ['재원', '강한울', '이승찬', '정민석', '정진규'][Math.floor(Math.random() * 5)]
      const randomYear = [2021, 2022, 2023, 2024, 2025][Math.floor(Math.random() * 5)]
      const randomPeriod = ['h1', 'h2'][Math.floor(Math.random() * 2)]
      
      return {
        cardId: `temp_${randomMember}_${randomYear}_${randomPeriod}`,
        name: `${randomMember} ${randomYear}년 ${randomPeriod === 'h1' ? '상반기' : '하반기'}`,
        type: CardType.YEAR,
        rarity: CardRarity.BASIC,
        description: `${randomYear}년 ${randomPeriod === 'h1' ? '상반기' : '하반기'} ${randomMember}의 모습`,
